...
Запитання для перевірки: чи вся нова кулька точно всередині поля, якщо натиснути дуже близько до межі? Зараз ми перевіряємо тільки точку кліку, тобто центр кульки. Для сильніших: створіть `Ball newBall = makeBallAt(point)` і змініть перевірку так, щоб у полі були і `point.x - newBall.radius`, і `point.x + newBall.radius`, і відповідні межі по `y`. Додавайте `newBall` у `balls` тільки після цієї перевірки.
