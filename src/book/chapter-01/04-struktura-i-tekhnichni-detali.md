...
:::details Орієнтовні відповіді
1. `pollEvent()` викликають у внутрішньому циклі, бо за один кадр у черзі може накопичитися кілька подій. Цикл забирає їх усі перед оновленням і малюванням кадру.
2. `KeyPressed` - це разова подія в момент натискання клавіші. `sf::Keyboard::isKeyPressed(...)` перевіряє поточний стан клавіші кожен кадр, поки її утримують.
3. `window.draw(...)` лише готує кадр у прихованому буфері. Без `window.display()` підготовлений кадр не стане видимим у вікні.
4. `clear` очищає старий кадр, `draw` додає об'єкти нового кадру, а `display` показує готовий результат. Якщо змінити порядок або пропустити крок, можна побачити сліди старих кадрів або не побачити новий кадр взагалі.
:::
